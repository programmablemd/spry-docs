...
<Steps>
  <Step>
    ...
    Familiarize yourself with the [Development Setup](./development-setup) guide to understand the project structure and tooling.
  </Step>
...
```bash
git commit -m "feat(axiom): add custom edge rule support"
git commit -m "fix(runbook): resolve dependency cycle detection"
git commit -m "docs: update contributing guide"
git commit -m "test(task): add execution plan tests"
git commit -m "refactor(shell): simplify command parsing"
```
