...
<Steps>
  <Step>
    ...
    | `import_map.json` | Deno import mappings for dependencies |

    <Callout type="info">
    The `spry.ts` file makes it easy to run Spry commands without typing the full Deno command each time. Just use `./spry.ts` instead.
    </Callout>
  </Step>
...
```bash build --depends clean --descr "Build the project"
echo "Building..."
...
```bash deploy --depends build --descr "Deploy the application"
echo "Deploying..."
cat output/app.txt
...
| `--depends task1,task2` | Task dependencies | `--depends clean,test` |
...
````markdown
```bash test --descr "Run test suite" --depends build --silent
npm test
```
````